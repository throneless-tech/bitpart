...
impl<'a> Data<'a> {
    ...
    // type_complexity: snapshots the scope fields for restoration; the tuple is
    // immediately destructured by the single caller, so a named struct would add
    // a type without improving the call site.

...
impl Primitive for PrimitiveArray {
    ...
    async fn do_exec(
        ...
        match name {
            "map" => {
                let res = PrimitiveArray::map(self, args, interval, data, msg_data, sender).await?;
                return Ok((res, Right::Read));
            }
            "filter" => {
                let res =
                    PrimitiveArray::filter(self, args, interval, data, msg_data, sender).await?;
                return Ok((res, Right::Read));
            }
            "reduce" => {
                let res =
                    PrimitiveArray::reduce(self, args, interval, data, msg_data, sender).await?;
                return Ok((res, Right::Read));
            }
            _ => {}
        }

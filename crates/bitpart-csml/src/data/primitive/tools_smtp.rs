...
pub fn get_mailer(
    ...
    data: &Data,
    interval: Interval,
) -> Result<AsyncSmtpTransport<Tokio1Executor>, ErrorInfo> {
    ...
    match is_tls {
        true => {
            let smtp_builder = match starttls {
                true => AsyncSmtpTransport::<Tokio1Executor>::starttls_relay(smtp_server),
                _ => AsyncSmtpTransport::<Tokio1Executor>::relay(smtp_server),
            };

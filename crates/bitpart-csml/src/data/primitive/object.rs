    ...
    async fn send_http(
        ...
        if let Some(literal) = object.value.get("method") {
            ...
            let (value, response_info) =
                http_request(&object.value, method, &data.context.flow, interval, false).await?;
...
impl PrimitiveObject {
    fn create_hmac(
        ...
        let signature = tools_crypto::sign_rsa(algo, key, data.as_bytes(), flow_name, interval)?;
    ...
    fn create_hash(
        ...
        let digest_bytes = tools_crypto::hash_data(algo, data.as_bytes());
...
impl Primitive for PrimitiveObject {
    ...
    fn format_mem(&self, content_type: &str, first: bool) -> serde_json::Value {
        ...
        match (content_type, first) {
            ("object", false) => PrimitiveObject::obj_literal_to_json(&self.value),
            (content_type, _) => {

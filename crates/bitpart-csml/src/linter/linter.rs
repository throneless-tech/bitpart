...
// type_complexity: the return tuple (loop path, interval, message) is local to
// the infinite-loop detection and used at one call site; a named alias would add
// indirection without clarifying intent.
...
fn infinite_loop_check(
    ...
    // we don't need to log non existent steps here because validate_gotos already do the work

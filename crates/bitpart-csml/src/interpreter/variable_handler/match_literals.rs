        ...
        (Some(array), Some(string)) => {
            for elem in array.value.iter() {
                match elem.primitive.as_any().downcast_ref::<PrimitiveString>() {
                    Some(val) if val.value.eq_ignore_ascii_case(&string.value) => return true,
                    _ => continue,
                }
            }
            false
...
pub fn match_obj(lit1: &Literal, lit2: &Literal) -> bool {
    match (&lit1.content_type, &lit2.content_type) {
        ...
        // op_ref: comparing the boxed `dyn Primitive` by value would move out
        // of the shared borrows; the references are required here.
        #[allow(clippy::op_ref)]

...
pub async fn resolve_object(
    ...
    match get_type(name, interval, data) {
        ObjType::NativeComponent => {
            let resolved_args =
                resolve_fn_args(args, data, msg_data, &DisplayWarnings::On, sender).await?;
        ...
        ObjType::BuiltInWithoutWarnings => {
            let resolved_args =
                resolve_fn_args(args, data, msg_data, &DisplayWarnings::Off, sender).await?;
...
pub async fn exec_closure(
    ...
    Box::pin(exec_fn_in_new_scope(scope, data, msg_data, sender)).await
}

...
> {
    match expr {
        Expr::PathExpr { literal, path } => {
            Box::pin(get_var_info(literal, Some(path), data, msg_data, sender)).await
        }
...
pub async fn match_actions(
    ...
    match function {
        ...
        ObjectType::Debug(args, interval) => {
            let args =
                resolve_fn_args(args, data, &mut msg_data, &DisplayWarnings::On, sender).await?;
        ...
        ObjectType::Log {
            ...
            let args =
                resolve_fn_args(expr, data, &mut msg_data, &DisplayWarnings::On, sender).await?;
        ...
        ObjectType::Use(arg) => {
            expr_to_literal(arg, &DisplayWarnings::On, None, data, &mut msg_data, sender).await?;
            Ok(msg_data)
        }
        ObjectType::Do(DoType::Update(assign_type, old, new)) => {
            ...
            let (lit, name, mem_type, path) =
                get_var_info(old, None, data, &mut msg_data, sender).await?;
        ...
        ObjectType::Goto(GotoType::Step(step), interval) => {
            let step = search_goto_var_memory(step, &mut msg_data, data, sender).await?;
        ...
        ObjectType::Goto(GotoType::Flow(flow), ..) => {
            let flow = search_goto_var_memory(flow, &mut msg_data, data, sender).await?;
        ...
        ObjectType::Goto(
            ...
            let step = match step {
                Some(step) => search_goto_var_memory(step, &mut msg_data, data, sender).await?,
                None => "start".to_owned(), // default value start step
            };
            let flow = match flow {
                Some(flow) => search_goto_var_memory(flow, &mut msg_data, data, sender).await?,
        ...
        ObjectType::Goto(
            ...
            let step = match step {
                Some(step) => ContextStepInfo::UnknownFlow(
                    search_goto_var_memory(step, &mut msg_data, data, sender).await?,
                ),
            ...
            let bot = search_goto_var_memory(next_bot, &mut msg_data, data, sender).await?;

...
async fn loop_path(
    ...
    mem_type: &MemoryType,
    new: Option<Literal>,
    path: &mut Iter<'_, (Interval, PathLiteral)>,
    ...
    for (interval, node) in path.iter() {
        match node {
            PathState::ExprIndex(expr) => {
                let lit = expr_to_literal(expr, dis_warnings, None, data, msg_data, sender).await?;
            ...
            PathState::Func(Function {
                ...
                let resolved_args =
                    Box::pin(resolve_fn_args(args, data, msg_data, dis_warnings, sender)).await?;
                new_path.push((
                    interval.to_owned(),
                    PathLiteral::Func {
                        name: name.to_owned(),
...
pub async fn exec_path_actions(
    lit: &mut Literal,
    dis_warnings: &DisplayWarnings,
    mem_type: &MemoryType,
    new: Option<Literal>,
...
pub async fn get_metadata_context_literal(
...
pub async fn get_literal_from_metadata(
    path: &[(Interval, PathLiteral)],
    dis_warnings: &DisplayWarnings,
    data: &mut Data<'_>,
    msg_data: &mut MessageData,
...
pub async fn get_var(
    ...
    match var.ident {
        name if name == COMPONENT => {
            gen_literal_from_component(*interval, path, data, msg_data, sender).await
        }
        name if name == EVENT => {
            gen_literal_from_event(*interval, dis_warnings, path, data, msg_data, sender).await
        }
        name if name == _ENV => match path {
            Some(path) => {
                let path =
                    Box::pin(resolve_path(path, dis_warnings, data, msg_data, sender)).await?;
        ...
        name if name == _METADATA => match path {
            Some(path) => {
                let path =
                    Box::pin(resolve_path(path, dis_warnings, data, msg_data, sender)).await?;
                get_literal_from_metadata(&path, dis_warnings, data, msg_data, sender).await
            }
            None => {
        ...
        _ => {
            ...
            match get_var_from_mem(var.to_owned(), dis_warnings, path, data, msg_data, sender).await
            {
                ...
                Err(err) => {
                    ...
                    let path = if let Some(p) = path {
                        Some(Box::pin(resolve_path(p, dis_warnings, data, msg_data, sender)).await?)
                    } else {
                        None
                    };
...
pub async fn get_var_from_mem<'a>(
    name: Identifier,
    dis_warnings: &DisplayWarnings,
    path: Option<&[(Interval, PathState)]>,
    data: &'a mut Data<'_>,
...
pub async fn search_goto_var_memory(
    var: &GotoValueType,
    msg_data: &mut MessageData,
    data: &mut Data<'_>,
    sender: &Option<mpsc::Sender<MSG>>,
    ...
    match var {
        ...
        GotoValueType::Variable(expr) => {
            let literal =
                expr_to_literal(expr, &DisplayWarnings::On, None, data, msg_data, sender).await?;
...
pub async fn get_string_from_complex_string(
    exprs: &[Expr],
    interval: Interval,
    data: &mut Data<'_>,
    msg_data: &mut MessageData,
    ...
    for elem in exprs.iter() {
        match expr_to_literal(elem, &DisplayWarnings::On, None, data, msg_data, sender).await {
            Ok(var) => {

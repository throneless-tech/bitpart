...
async fn interpret_function_scope(
    actions: &Block,
    data: &mut Data<'_>,
    ...
    for (action, instruction_info) in actions.commands.iter() {
        match action {
            ...
            Expr::ForEachExpr(ident, i, expr, block, range) => {
                message_data =
                    for_loop(ident, i, expr, block, range, message_data, data, sender).await?
            }
            Expr::WhileExpr(expr, block, range) => {
                message_data = while_loop(expr, block, range, message_data, data, sender).await?
            }
            e => {
...
pub async fn exec_fn_in_new_scope(
    expr: &Expr,
    new_scope_data: &mut Data<'_>,
    ...
    match expr {
        Expr::Scope {
            ...
            let fn_msg_data =
                Box::pin(interpret_function_scope(scope, new_scope_data, sender)).await?;

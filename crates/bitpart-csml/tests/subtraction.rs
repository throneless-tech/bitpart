// assertions_on_constants: these tests use the `if ok { return assert!(true) } assert!(false)`
// idiom as a pass/fail marker; rewriting each condition risks changing test intent.
